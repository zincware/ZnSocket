...
def test_chunking_stress_many_operations(client, request, caplog):
    ...
    for dict_idx in range(num_dicts):
        dct = znsocket.Dict(
            r=c, key=f"stress_test_dict_{dict_idx}", converter=[NumpyConverter]
        )
        ...
        np.testing.assert_array_equal(
            dct[f"array_{arrays_per_dict - 1}"], arrays[f"array_{arrays_per_dict - 1}"]
        )
    ...
    print(
        f"✅ Stress test completed: {num_dicts} dicts × {arrays_per_dict} arrays in {duration:.2f}s"
    )
...
def test_chunking_very_large_single_item(client, request, caplog):
    ...
    print(
        f"✅ Very large single item ({huge_array.nbytes:,} bytes) handled in {end_time - start_time:.2f}s"
    )
...
def test_chunking_boundary_conditions(client, request, caplog):
    ...
    for name, target_size in test_sizes:
        ...
        print(
            f"{name:15} | Target: {target_size:>10,} | Actual: {actual_size:>10,} | "
        ...
        if target_size > threshold:
            assert chunked, (
                f"Data of size {actual_size} should be chunked (threshold: {threshold})"
            )
