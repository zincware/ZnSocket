...
def test_chunking_nested_data_structures(client, request, caplog):
    ...
    nested_data = {
        "level1": {
            ...
            "additional_data": [
                np.random.rand(100, 100).astype(np.float64) for _ in range(5)
            ],
        }
    }
...
def test_chunking_size_threshold_boundary(client, request, caplog):
    ...
    small_array = np.random.rand(under_threshold_size, under_threshold_size).astype(
        np.float64
    )

    # Create data well over the threshold
    over_threshold_size = int(
        np.sqrt(threshold // 8 * 2)
    )  # Should definitely trigger chunking
    large_array = np.random.rand(over_threshold_size, over_threshold_size).astype(
        np.float64
    )
