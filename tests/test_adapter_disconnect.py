...
@pytest.mark.parametrize("server", ["eventlet_memory_server", "eventlet_memory_server_redis"])
def test_adapter_disconnect_cleanup(server, request, list_data):
    ...
    list_adapter_key = f"znsocket.List:disconnect_test"
    dict_adapter_key = f"znsocket.Dict:znsocket.List:disconnect_test:0"
    ...
    assert exists_list_after is False, "List adapter should be cleaned up after client disconnect"
    assert exists_dict_after is False, "Dict adapter should be cleaned up after client disconnect"
