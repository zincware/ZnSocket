...
from znsocket.storages import MongoStorage, NativeStorage
...
class Server:
    ...
    def run(self) -> None:
        ...
        elif self.storage.startswith("mongodb://"):
            # Parse MongoDB connection string to extract database name
            if "/" in self.storage.split("://")[1]:
                connection_string, database_name = self.storage.rsplit("/", 1)
                resolved_storage = MongoStorage(connection_string + "/", database_name)
            else:
                resolved_storage = MongoStorage(self.storage, "znsocket")
