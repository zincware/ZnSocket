...
import { Dict, createClient } from "znsocket";
...
beforeEach(async () => {
  ...
  dct = new Dict({ client: client, key: "dict:test", fallback: "dict:test:fallback", fallbackPolicy: "frozen" });
});
...
test("test_dict_fallback_frozen_items", async () => {
  ...
  expect(items.sort()).toEqual([["a", 1], ["b", 2], ["c", 3]]);
});
